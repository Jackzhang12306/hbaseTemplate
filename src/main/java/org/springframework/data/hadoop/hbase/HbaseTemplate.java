...
//	@Override
	public  <T> T get(String tableName, final List<Get> gets, final GetResultsExtractor<T> action) {
		return execute(tableName, new TableCallback<T>() {
			@Override
			public T doInTable(HTableInterface htable) throws Throwable {
				Result[] scanner = htable.get(gets);
//				try {
					return action.extractData(scanner);
//				} finally {
//					scanner.close();
//				}
	...
	@Override
	public <T> List<T>  get(String tableName, final List<String> rowName, final String familyName,final String qualifier, RowMapper<T> mapper) {
		Get get = null;
		 List<Get> gets = new ArrayList<>();
		for(String rowkey:rowName){
		 ...
		 return get( tableName,  gets, new  RowMapperGetResultsExtractor<T> (mapper));
		
	}

	@Override
	public <T> List<T> get(String tableName, List<String> rowName,
			String familyName, RowMapper<T> mapper) {
		return  get( tableName,  rowName, familyName,null, mapper) ;
	}
